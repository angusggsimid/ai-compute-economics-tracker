...
线上部署链路已经与本地 DuckDB 解耦：OpenRouter、Foundry、模型价格和 CAPEX 均先落入 `tracker_data/backfills/*.json`，再由正式构建器生成 `public/index.html`。GitHub Actions 负责每日刷新、测试和提交；push 到 main 后 Vercel 与 EdgeOne Pages 各自通过 GitHub 集成自动部署（EdgeOne 项目 `ai-compute-economics-tracker` 绑定 main 分支，输出目录 `public/`，无构建步骤），数据更新不依赖用户电脑开机。`scripts/backfill_gpu_orderbook.py` 另行按日累积 GPUPerHour/Vast/RunPod 订单簿深度汇总（offer 数、GPU 数、P25/P50/P75），写入 `tracker_data/backfills/gpu_orderbook_history.json`：它是时点观测的积累型信息源（当天抓不到即永久缺口），当前不进入正式页面，单源失败记录进 quality、全部失败标为 `failed` 并告警，但不阻塞主链路发布；未来接入 Supply Price 深度证据或页面展示时再升级为硬门槛。

发布质量门由 `scripts/validate_deploy_refresh.py` 统一执行。OpenRouter、Foundry 和活跃模型牌价必须为当天 `fresh`；SEC CAPEX 保留季度自然频率，抓取失败时只有五家公司最近官方值均在 150 天内才允许以 `current_for_frequency` 继续发布。任何来源缺失、缓存超期或页面测试失败都会返回非零状态，GitHub 不提交，Vercel 保留上一版。
