...
线上部署链路已经与本地 DuckDB 解耦：OpenRouter、Foundry、模型价格和 CAPEX 均先落入 `tracker_data/backfills/*.json`，再由正式构建器生成 `public/index.html`。GitHub Actions 负责每日刷新、测试和提交；Vercel 监听主分支自动发布并只托管 `public/`，提交推送后同一流水线再用 EdgeOne CLI 将 `public/` 直接上传到 EdgeOne Pages 项目 `ai-compute-tracker`（token 存于仓库 secret），数据更新不依赖用户电脑开机。
